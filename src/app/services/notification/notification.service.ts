...
export class NotificationService {
  ...
  async init() {
    ...
    this.serviceWorkerRegistration = await navigator.serviceWorker.register(`firebase-messaging-sw.bundled.js`, {
      type: 'module',
      scope: ''
    });
  ...
  // call this methode to clean resources used by this service (when signing-out, so that the use doesn't keep receiving notification while being logged-out)
  async uninit() {
    if (this.serviceWorkerRegistration !== undefined) {
      try {
